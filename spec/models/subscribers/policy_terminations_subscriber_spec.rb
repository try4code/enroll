...
describe Subscribers::PolicyTerminationsSubscriber do
  ...
  describe "given a termination event with qualify reason non_payment, for an already terminated enrollment" do
    let(:termination_date) { Date.new(2017, 5, 30) }
    ...
    before :each do
      ...
      allow(existing_enrollment).to receive(:may_terminate_for_non_payment?).and_return(false)
    end

    it "does nothing to the enrollment" do
  ...
  describe "given a cancel event with qualify reason non_payment, for an already canceled enrollment" do
    let(:termination_payload) do
      {
        :resource_instance_uri => "urn:some_thing:policy#policy_id",
        :hbx_enrollment_ids => JSON.dump([enrollment_id]),
    ...
    before :each do
      ...
      allow(existing_enrollment).to receive(:may_cancel_for_non_payment?).and_return(false)
    end

    it "does nothing to the enrollment" do
      expect(existing_enrollment).not_to receive(:cancel_for_non_payment!)
      expect(existing_enrollment).not_to receive(:cancel_coverage!)
      subject.call(cancelation_event_name, nil, nil, nil, termination_payload)
    end
  end
end
