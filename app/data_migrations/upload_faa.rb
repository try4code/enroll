    ...
    if File.exist?(file)
      CSV.foreach("#{Rails.root}/hbx_report/families_with_application.csv", {encoding: "UTF-8", headers: true, header_converters: :symbol, converters: :all}) do |application|
        ...
        if !applications_in_draft.present?
              ...
              if app_family_member_id == applicant.to_hash[:family_member_id]
                ...
                CSV.foreach("#{Rails.root}/hbx_report/families_with_applicants_benefit.csv", {encoding: "UTF-8", headers: true, header_converters: :symbol, converters: :all}) do |benefit|
                  if benefit.present? && app_family_member_id == benefit.to_hash[:family_member_id]
                    ...
                    #employer address creation
                    CSV.foreach("#{Rails.root}/hbx_report/families_with_applicants_benefit_er_address.csv", {encoding: "UTF-8", headers: true, header_converters: :symbol, converters: :all}) do |address|
                      if address.present? && app_family_member_id == address.to_hash[:family_member_id] && benefit.to_hash[:benefit_id] == address.to_hash[:benefit_id]
                        new_address = new_benefit.build_employer_address(
                            kind: address.to_hash[:kind],
                            address_1: address.to_hash[:address_1],
                            address_2: address.to_hash[:address_2],
                            address_3: address.to_hash[:address_3],
                    ...
                    #employer phone creation
                    CSV.foreach("#{Rails.root}/hbx_report/families_with_applicants_benefit_er_phone.csv", {encoding: "UTF-8", headers: true, header_converters: :symbol, converters: :all}) do |phone|
                      if phone.present? && app_family_member_id == phone.to_hash[:family_member_id] && benefit.to_hash[:benefit_id] == phone.to_hash[:benefit_id]
                        new_phone = new_benefit.build_employer_phone(kind: phone.to_hash[:kind],
                                                                     country_code: phone.to_hash[:country_code],
                                                                     area_code: phone.to_hash[:area_code],
                                                                     number: phone.to_hash[:number],
                                                                     extension: phone.to_hash[:extension],
          ...
          puts "Family with id:#{f_id} already have application in draft state}" unless Rails.env.test?
        end
      end

      puts "----uploaded from CSV to Enroll app: FAA----" unless Rails.env.test?
      puts "applications created #{applications_count}" unless Rails.env.test?
      puts "applicants created #{applicants_count}" unless Rails.env.test?
      puts "incomes created #{incomes_count}" unless Rails.env.test?
      puts "income - employer addresses created #{incomes_er_address_count}" unless Rails.env.test?
      puts "income - employer phones created #{incomes_er_phone_count }" unless Rails.env.test?
      puts "benefits created #{benefits_count}" unless Rails.env.test?
      puts "benefit - employer addresses created #{benefit_er_address_count}" unless Rails.env.test?
      puts "benefit - employer phones created #{benefit_er_phone_count }" unless Rails.env.test?
      puts "deductions created #{deductions_count}" unless Rails.env.test?
    else
      puts "CSV files are not found. Please generate CSV files using the download_fa_applications rake task" unless Rails.env.test?
    end
  end
end